...
export default function MenuUpcoming({info}: {info: StoreInfo}) {
  ...
  return <><Header initialDark reservationUrl="/reserve" reservationUrlEn="/en/reserve" />
    <main className="editorial-page"><section className="editorial-wrap menu-upcoming">
      <nav className="menu-breadcrumb" aria-label={en?'Breadcrumb':'パンくずリスト'}>
        ...
        <Link href={localize(`/stores/${info.slug}`)}>{en?info.nameEn||info.name:info.name}</Link>
      </nav>
      ...
      <div className="editorial-actions"><Link className="editorial-button" href={localize(`/stores/${info.slug}`)}>{en?'Shop information & contact':'店舗情報・お問い合わせ'} ↗</Link>
      <Link className="editorial-text-link" href={localize(`/reserve#${info.slug}`)}>{en?'Reservations':'ご予約のご案内'} ↗</Link></div>
    </section></main></>;
}
