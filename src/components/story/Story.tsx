...
import { artSrc, createStorySignal, CUTS, cutState, STORY_SCROLL_SVH } from "@/lib/storyScript";

const DepthCanvas = dynamic(() => import("./DepthCanvas"), { ssr: false });
const frames = CUTS.map(cut => ({ src: artSrc(cut.id), mobileSrc: artSrc(cut.id, true), depth: `/story-art/${cut.id}-depth.webp`, aspect: 1.5 }));
...
export default function Story() {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const state = cutState(progress);
      const preload = (i: number) => {
        const image = images.current[i];
        if (image && !image.getAttribute("src")) {
          image.srcset = `${artSrc(CUTS[i].id, true)} 960w, ${artSrc(CUTS[i].id)} 1536w`;
          image.src = artSrc(CUTS[i].id);
        }
      };
      preload(state.a); preload(state.b);
      if (state.b + 1 < CUTS.length && state.t > .3) preload(state.b + 1);
  ...
  if (flat) return <section ref={root} id="story" className="painted-story-static">
    <div className="editorial-wrap story-static-heading"><p className="eyebrow">THE CHASEN STORY</p><h1>{en ? "The story of a bowl of matcha." : "一杯が、点てられるまで。"}</h1><div className="editorial-actions">{!reduce && <button type="button" onClick={toggle} className="editorial-text-link">{en ? "Resume the immersive story" : "立体の物語に戻る"} ↗</button>}<a className="editorial-text-link" href="#stores">{en ? "Explore our shops" : "店舗・お品書きへ"} ↓</a></div></div>
    <div className="story-static-grid editorial-wrap">{CUTS.map((cut, i) => <figure key={cut.id}>
      {/* eslint-disable-next-line @next/next/no-img-element -- Same responsive file as WebGL, no second optimizer download. */}
      <img src={artSrc(cut.id)} srcSet={`${artSrc(cut.id, true)} 960w, ${artSrc(cut.id)} 1536w`} sizes="(max-width: 767px) 100vw, 50vw" width={1536} height={1024} alt={en ? cut.detailEn : cut.detail} loading={i === 0 ? "eager" : "lazy"} />
      <figcaption><span>0{i + 1} / {cut.label}</span><p>{en ? cut.en : cut.ja}</p></figcaption>
    </figure>)}</div>
  </section>;

  return <section ref={root} id="story" className="painted-story" aria-label={en ? "Chasen, an illustrated scroll story" : "茶筅、背景画でめぐる物語"} style={{ height: `${STORY_SCROLL_SVH}svh` }}>
    {CUTS.map((cut, i) => <span key={cut.id} id={`scene-${cut.id}`} className="story-anchor" style={{ top: `${(i === 0 ? 0 : (i + .12) / CUTS.length) * (STORY_SCROLL_SVH - 100)}svh` }} />)}
    <div ref={sticky} className="painted-story-viewport">
      <div className="painted-story-images" aria-hidden="true">{CUTS.map((cut, i) =>
        /* eslint-disable-next-line @next/next/no-img-element -- HTML first paint and context-loss fallback share WebGL image URLs. */
        <img key={cut.id} ref={element => { images.current[i] = element; }} src={i < 2 ? artSrc(cut.id) : undefined} srcSet={i < 2 ? `${artSrc(cut.id, true)} 960w, ${artSrc(cut.id)} 1536w` : undefined} sizes="100vw" alt="" width={1536} height={1024} fetchPriority={i === 0 ? "high" : "auto"} onLoad={() => refresh.current()} style={{ opacity: i === 0 ? 1 : 0 }} />
      )}</div>
      <DepthCanvas frames={frames} signal={signal} />
      ...
      <div className="story-caption-stack">{CUTS.map((cut, i) => <div key={cut.id} className="story-caption" data-active={active === i} aria-hidden={active !== i}>
        <p className="story-kicker">0{i + 1}<span />{cut.label}</p>
        {i === 0 ? <h1>{en ? <>The story of<br />a bowl of matcha.</> : <>一杯が、<br />点てられるまで。</>}</h1> : <h2>{en ? cut.en : cut.ja}</h2>}
        <p className="story-detail">{en ? cut.detailEn : cut.detail}</p>
      </div>)}</div>
      <div className="story-controls">
        <nav aria-label={en ? "Story scenes" : "物語の場面"} className="story-chapters">{CUTS.map((cut, i) => <a key={cut.id} href={`#scene-${cut.id}`} aria-label={`${i + 1}. ${en ? cut.en : cut.ja}`} aria-current={active === i ? "step" : undefined}><span className="story-chapter-line" /><span>0{i + 1}</span></a>)}</nav>
